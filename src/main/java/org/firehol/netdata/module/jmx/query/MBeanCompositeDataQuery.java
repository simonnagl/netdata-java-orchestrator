...
class MBeanCompositeDataQuery extends MBeanQuery {
    ...
    public void addDimension(Dimension dimension, final String attribute) throws JmxMBeanServerQueryException {
        ...
        allDimensionByKey.computeIfAbsent(splitString[1], k -> MBeanValueStore.newInstance(result)).addDimension(dimension);
    }

    @Override
    public List<Dimension> getDimensions() {
        return allDimensionByKey.values().stream().flatMap(MBeanValueStore::streamAllDimension).collect(Collectors.toList());
    }
